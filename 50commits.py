...
class User:
    ...
    def check_achievements(self):
        # Novo achievement: Primeiro Commit
        if self.commits == 1 and "Primeiro Commit!" not in self.achievements:
            self.unlock_achievement("Primeiro Commit!", "🚀 Você deu o primeiro passo!")

        # Novo achievement: 50 commits
        if self.commits == 50 and "Commitando sem parar" not in self.achievements:
            self.unlock_achievement("Commitando sem parar", "🔥 50 commits feitos com sucesso!")
